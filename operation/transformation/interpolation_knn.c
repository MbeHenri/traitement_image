...
int main(int argc, char const *argv[])
{
    ...
    {
        f=fopen("helpAll/interpolation_knn.help", "r");
        int car = 0;
        if (f != NULL)
        {
    ...
    }else if (strcmp(ch, "P2") == 0)
    {
        ...
        {
            free_ImageG(img);
            printf("[ donnez le nombre k d'agrandissement ]\n");
            exit(1);
        }
        ...
        {
            free_ImageG(img);
            printf("[ mauvais nombre k d'agrandissement ]\n");
            exit(1);
        }
        ...
        if (argv[3] == NULL)
        {
            ...
            dest1 = malloc((14+strlen(info->name)+strlen(argv[2])+strlen(current_dir))*sizeof(char));
            dest1[0]='\0';
        ...
        {
            printf("...");
            ImageG* r = interpolation_knnG(img, k);
            printf("\n");
            write_G(r, dest1);
            free_ImageG(r);
            free(dest1);
