...
int main(int argc, char const *argv[])
{
    ...
    {
        f=fopen("helpAll/convoluerMoyenne.help", "r");
        int car = 0;
        if (f != NULL)
        {
    ...
    else if (strcmp(ch, "P2") == 0)
    {
        ...
        if (argv[3] == NULL)
        {
            ...
            dest1 = malloc((19+strlen(info->name)+strlen(argv[2])+strlen(current_dir))*sizeof(char));
            dest1[0]='\0';
            strcat(dest1,current_dir);
            strcat(dest1,"/");
            strcat(dest1,info->name);
            strcat(dest1,"-convMoy(");
            strcat(dest1,argv[2]);
            strcat(dest1,").pgm");
            free_i_file(info);
            free(current_dir);
        ...
        {
            printf("...");
            ImageG *r = convolutionMoyG(img, pas);
            printf("\n");
            write_G(r, dest1);
            free_ImageG(r);
            free(dest1);
