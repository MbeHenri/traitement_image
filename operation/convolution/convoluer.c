...
int main(int argc, char const *argv[])
{
    ...
    {
        f=fopen("helpAll/convoluer.help", "r");
        int car = 0;
        if (f != NULL)
        {
    ...
    else if (strcmp(ch, "P2") == 0)
    {
        ...
        if (filtre == NULL)
        {
            printf("[ le filtre n'as pas pu etre chargé ]\n");
            free_ImageG(img);
            exit(1);
        }
        ...
        if (argv[3] == NULL)
        {
            ...
            dest1 = malloc((16+strlen(info->name)+strlen(info1->name)+strlen(current_dir))*sizeof(char));
            dest1[0]='\0';
