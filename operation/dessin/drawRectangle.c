...
int main(int argc, char const *argv[])
{
    ...
    {
        f=fopen("helpAll/drawrectangle.help", "r");
        int car = 0;
        if (f != NULL)
        {
    ...
    else if (strcmp(ch, "P2") == 0)
    {
        ...
        x1 = atoi(argv[2]); y1 = atoi(argv[3]); hauteur = atof(argv[4]); largeur = atof(argv[5]);couleur= atoi(argv[6]);
        if( x1 >= img->nColonne || x1<0 || hauteur <=0 || y1 >= img->nLigne || y1<0 || largeur <=0 || couleur<0 || couleur>255){
            free_ImageG(img);
        ...
        if (argv[7] == NULL)
        {
            ...
            dest1 = malloc((1+strlen(argv[7]))*sizeof(char));
            strcpy(dest1, argv[7]);
        }
        /* TRACER DU RECTANGLE)
         */
        if (dest1!=NULL)
        {
            drawRectangleG(img, x1, y1, hauteur, largeur, couleur);
            write_G(img, dest1);
            free(dest1);
        }
        free_ImageG(img);
